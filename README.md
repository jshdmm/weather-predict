...
This project was created as an extension of my [ELT pipepline project](https://github.com/phipsrick/GA2024_1) which covered integrating weather data with different IoT sensors from a Dutch household. 

**A LightGBM regressor predicting hourly temperature (°C) trained daily from [Open-Meteo](https://open-meteo.com/).**


**Importantly, feature engineering and model performance optimization were not the focus of this project, but rather hosting a continuously updated baseline model on [Hugging Face](https://huggingface.co/jshdmm/weather-predict-berlin) using CI/CD with GitHub Actions, automated testing, orchestration using a cronjob and containerization with Docker. The model can be accessed through FastAPI endpoints.** Besides reading through docs and tutorials, I used Claude Code as an assistant to work through the project. In doing so, I was still making relevant decisions myself (also see the commit history to follow along the project trajectory).
